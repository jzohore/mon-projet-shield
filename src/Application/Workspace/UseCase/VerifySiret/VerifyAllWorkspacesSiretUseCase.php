...
final readonly class VerifyAllWorkspacesSiretUseCase
{
    ...
    public function __invoke(): array
    {
        ...
        foreach ($workspaces as $workspace) {
            ...
            try {
                ($this->verifyWorkspaceSiretUseCase)($slug, 'cron@kysure.fr');
                ++$successCount;
            } catch (\Throwable $e) {
                ++$failureCount;
                ...
                $this->logger->error(sprintf('Échec du batch SIRET pour le workspace "%s": %s', $slug, $e->getMessage()), [
                    'workspace_slug' => $slug,
                    'exception' => $e,
                ]);
            }
